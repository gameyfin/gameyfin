...
--   SMALLINT        - supported by both
--   TEXT            - supported by both (H2 2.0+)
--   BYTEA           - supported by both (H2 2.2+ added BYTEA alias)
--   VARCHAR+CHECK   - replaces H2 ENUM; supported by both
--   TIMESTAMP WITH TIME ZONE - supported by both (H2 2.0+)
--   Unquoted identifiers in CHECK constraints - supported by both
...
-- BYTEA: supported by both H2 2.2+ and PostgreSQL
...
-- VARCHAR(255) + CHECK replaces H2 ENUM and is compatible with both H2 and PostgreSQL
CREATE TABLE USER_ROLES
(
    USER_ID BIGINT NOT NULL,
    ROLES   VARCHAR(255),
    CONSTRAINT CHK_USER_ROLES_VALID CHECK (ROLES IN ('ADMIN', 'SUPERADMIN', 'USER')),
    CONSTRAINT FKHFH9DX7W3UBF1CO1VDEV94G3F
        FOREIGN KEY (USER_ID) REFERENCES USERS
);

-- BIGINT GENERATED BY DEFAULT AS IDENTITY: SQL standard, supported by both H2 and PostgreSQL
